...
export default function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleDelete() {
    if (!job) return;
    if (!confirm(`Delete "${job.name}"? This also deletes its run history.`)) return;
    setDeleting(true);
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h1 className="text-2xl font-bold tracking-tight truncate">{job.name}</h1>
            {job.enabled ? (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="rounded-xl border border-border bg-surface p-4">
          <p className="text-xs uppercase tracking-wider text-text-dim mb-1">Method</p>
          <p className="font-mono text-sm">{job.method}</p>
        </div>
        <div className="rounded-xl border border-border bg-surface p-4">
        ...
        <div className="rounded-xl border border-border bg-surface p-4">
          ...
          {job.last_run_status && (
            <p className={`text-xs mt-0.5 ${job.last_run_status === "success" ? "text-success" : "text-danger"}`}>
              {job.last_run_status}
            </p>
          )}
        </div>
        <div className="rounded-xl border border-border bg-surface p-4">
          <p className="text-xs uppercase tracking-wider text-text-dim mb-1">Config</p>
          <p className="text-sm">
            Timeout: {job.timeout_seconds}s &middot; Retries: {job.retry_count}
          </p>
        </div>
      </div>
      ...
      {job.headers && Object.keys(job.headers).length > 0 && (
        <div className="rounded-xl border border-border bg-surface p-4">
          <p className="text-xs uppercase tracking-wider text-text-dim mb-2">Headers</p>
          <pre className="font-mono text-sm whitespace-pre-wrap text-text-dim">{JSON.stringify(job.headers, null, 2)}</pre>
        </div>
      )}
      ...
      <div>
        ...
        {runs.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border bg-surface/50 p-8 text-center">
            <p className="text-text-dim text-sm">No runs yet. The scheduler will execute this job based on its schedule.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {runs.map((run) => {
              ...
              return (
                <div
                  key={run.id}
                  className="flex items-center gap-4 rounded-lg border border-border bg-surface p-3 hover:bg-surface-hover transition-colors"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {run.status_code !== null && (
                        <span className="text-xs font-mono text-text-dim">HTTP {run.status_code}</span>
                      )}
                      <span className="text-xs text-text-dim">Attempt {run.attempt_number}</span>
                    </div>
                    {run.error_message && (
                      <p className="text-xs text-danger truncate mt-0.5">{run.error_message}</p>
                    )}
                  </div>
